...
// Only shown after the user completes registration
...
export default function Header() {
  ...
  // Check localStorage on mount and whenever the route changes
  useEffect(() => {
    ...
    <header className="sticky top-0 z-50 backdrop-blur-xl border-b" style={{ background: 'rgba(2,11,24,0.85)', borderColor: 'rgba(255,255,255,0.07)' }}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between h-16">
        ...
        <div className="flex items-center">
          <button className="lg:hidden p-2 rounded-lg text-slate-300 hover:text-white" onClick={() => setOpen(!open)}>
      ...
      {/* Mobile menu */}
      {open && (
        <div className="lg:hidden border-t px-4 py-3 space-y-1" style={{ background: 'rgba(2,11,24,0.95)', borderColor: 'rgba(255,255,255,0.07)' }}>
          {NAV.map(n => (
