...
const db = new Firestore({ projectId: process.env.GCP_PROJECT_ID || 'civiccompass-495119' });
...
const JOURNEY_STAGES = [
  ...
  { index: 2, label: 'Roll Verification', icon: 'search', color: '#0EA5E9', description: 'Confirm your name appears correctly on the official voter list.', tips: 'Check online or at your local ERO (Electoral Registration Officer) office.' },
...
const STATE_TIMELINES = [
  { state: 'delhi', stateName: 'Delhi', notificationDate: '2024-10-15', nominationDeadline: '2024-10-22', lastWithdrawal: '2024-10-25', pollDate: '2024-11-20', resultDate: '2024-11-23', pollProximityDays: 25 },
  { state: 'maharashtra', stateName: 'Maharashtra', notificationDate: '2024-10-01', nominationDeadline: '2024-10-08', lastWithdrawal: '2024-10-11', pollDate: '2024-11-05', resultDate: '2024-11-08', pollProximityDays: 10 },
  { state: 'gujarat', stateName: 'Gujarat', notificationDate: '2024-11-01', nominationDeadline: '2024-11-08', lastWithdrawal: '2024-11-11', pollDate: '2024-12-05', resultDate: '2024-12-08', pollProximityDays: 40 },
  { state: 'karnataka', stateName: 'Karnataka', notificationDate: '2024-09-20', nominationDeadline: '2024-09-27', lastWithdrawal: '2024-09-30', pollDate: '2024-10-28', resultDate: '2024-10-31', pollProximityDays: 5 },
  { state: 'rajasthan', stateName: 'Rajasthan', notificationDate: '2024-11-10', nominationDeadline: '2024-11-17', lastWithdrawal: '2024-11-20', pollDate: '2024-12-15', resultDate: '2024-12-18', pollProximityDays: 50 },
  { state: 'up', stateName: 'Uttar Pradesh', notificationDate: '2024-10-05', nominationDeadline: '2024-10-12', lastWithdrawal: '2024-10-15', pollDate: '2024-11-10', resultDate: '2024-11-13', pollProximityDays: 15 },
  { state: 'tamilnadu', stateName: 'Tamil Nadu', notificationDate: '2024-09-01', nominationDeadline: '2024-09-08', lastWithdrawal: '2024-09-11', pollDate: '2024-10-10', resultDate: '2024-10-13', pollProximityDays: -5 },
  { state: 'westbengal', stateName: 'West Bengal', notificationDate: '2024-11-20', nominationDeadline: '2024-11-27', lastWithdrawal: '2024-11-30', pollDate: '2024-12-25', resultDate: '2024-12-28', pollProximityDays: 60 },
  { state: 'punjab', stateName: 'Punjab', notificationDate: '2024-10-20', nominationDeadline: '2024-10-27', lastWithdrawal: '2024-10-30', pollDate: '2024-11-25', resultDate: '2024-11-28', pollProximityDays: 30 },
  { state: 'andhrapradesh', stateName: 'Andhra Pradesh', notificationDate: '2024-11-05', nominationDeadline: '2024-11-12', lastWithdrawal: '2024-11-15', pollDate: '2024-12-10', resultDate: '2024-12-13', pollProximityDays: 45 },
];
...
const MYTH_FACTS = [
  ...
  { id: 'mf008', myth: 'I can be bribed to vote for a party without consequences.', fact: 'Accepting bribes for votes is a criminal offence under the Representation of the People Act, 1951. Both the giver and receiver can be prosecuted.', category: 'legal' },
  ...
  { id: 'mf011', myth: 'I need to take a holiday to vote — it is not a paid leave.', fact: 'Election day is a public holiday for employees. The Model Code of Conduct also allows workers time off to vote.', category: 'process' },
  { id: 'mf012', myth: 'The VVPAT slip I see is my actual ballot paper.', fact: 'The VVPAT slip is only a visual confirmation record. It drops into a sealed box automatically and is not your ballot. The EVM records your vote.', category: 'technology' },
];
...
const CHECKLIST_TEMPLATES = [
  ...
  {
    type: 'registered',
    title: 'Already Registered Voter Checklist',
    items: [
...
async function seed() {
  console.log('🌱 Starting Firestore seed for CivicCompass Phase 1...\n');

  // Journey Stages
  for (const stage of JOURNEY_STAGES) {
    await db.collection('journeyStages').doc(`stage_${stage.index}`).set(stage);
    console.log(`  ✓ Journey stage: ${stage.label}`);
  }

  // State Timelines
  for (const tl of STATE_TIMELINES) {
    await db.collection('stateTimelines').doc(tl.state).set(tl);
    console.log(`  ✓ Timeline: ${tl.stateName}`);
  }

  // Myth/Facts
  for (const mf of MYTH_FACTS) {
    await db.collection('mythFacts').doc(mf.id).set(mf);
    console.log(`  ✓ Myth/Fact: ${mf.id}`);
  }

  // Checklists
  for (const cl of CHECKLIST_TEMPLATES) {
    await db.collection('checklistTemplates').doc(cl.type).set(cl);
    console.log(`  ✓ Checklist: ${cl.type}`);
  }

  // Booths
  for (const b of BOOTHS) {
    await db.collection('booths').doc(b.boothId).set(b);
    console.log(`  ✓ Booth: ${b.boothId} — ${b.name}`);
  }

  // Voting Simulation
  await db.collection('votingSimulation').doc(VOTING_SIMULATION.docId).set({ steps: VOTING_SIMULATION.steps });
  console.log(`  ✓ Voting simulation steps`);
